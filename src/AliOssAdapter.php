...
class AliOssAdapter extends AbstractAdapter
{
    ...
    public function listDirObjects($dirname = '', $recursive = false)
    {
        ...
        while (true) {
            $options = [
                'delimiter' => $delimiter,
                'prefix'    => empty($dirname) ? '' : (substr($dirname, -1) == '/' ? $dirname : ($dirname . '/')),
                'max-keys'  => $maxkeys,
                'marker'    => $nextMarker,
            ];
    ...
    public function listContents($directory = '', $recursive = false)
    {
        ...
        $contents   = [];
        if (!empty($dirObjects['prefix'])) {
            foreach ($dirObjects['prefix'] as $v) {
                $contents[] = [
                    'Key' => $v,
                ];
            }
        }
        $contents = array_merge($contents, $dirObjects['objects']);
